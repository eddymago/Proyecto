...
public class RequestStepClean {
    ...
    public void usingTokenInTodoLy() {
        ...
        String credentials = Base64.getEncoder()
                .encodeToString((Config.user+":"+Config.password).getBytes());

        requestInfo.setUrl(Config.host+"/api/authentication/token.json")
    ...
    public void sendPOSTRequestWithBody(String method,String url,String body) {
        requestInfo.setUrl(Config.host+this.replaceValues(url)).setBody(body);
        response = FactoryRequest.make(method).send(requestInfo);
    }

    @Then("response code should be {int}")
    public void responseCodeShouldBe(int expectCode) {
    ...
    public void saveInTheVariable(String attibute, String nameVariable) {

        variables.put(nameVariable,response.then().extract().path(attibute)+"");
    }
